...
export default function UpdateAnswer() {
  ...
  const fetchAnswer = async (id) => {
    const response = await fetch(`https://doubtout-backend.onrender.com/api/answer/userAnstoUpdate/${id}`, {
      method: "POST",
      headers: {
  ...
  const updateAns = async (e, id) => {
    ...
    let response = await fetch(`https://doubtout-backend.onrender.com/api/answer/updateans/${id}`, {
      method: "POST",
      headers: {
        'Content-Type': 'application/json'
      },
